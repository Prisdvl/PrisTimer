...
- **四套主题**，`@property` 颜色插值实现 0.9s 丝滑过渡，每套主题有专属背景动效；表盘指针、刻度与盘内粒子同主题换色（前景与底色明度反向，保证任何主题下辨识度）
...
- **表盘粒子**：计时运行时粒子在盘面内漂移连线，出界沿法线反弹；颜色随主题取色（深空靛蓝星光 / 虚空银白 / 晨雾墨蓝 / 极光春绿），状态色交给盘框、进度弧与呼吸光承担
